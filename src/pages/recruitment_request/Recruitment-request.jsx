...
export default function Recruitment_request() {
  ...
  const handleDownload = (workerId) => {
    if (!user) {
      alert("من فضلك قم بتسجيل الدخول لحجز السيرة الذاتية");
      return;
    }

    fetch(`/api/workers/${workerId}/cv`, {
      headers: {
        Authorization: `Bearer ${user.token}`, // التوكن من تسجيل الدخول
      },
    })
      .then((res) => {
        ...
        <div className="cv_inputs container">
          ...
          {url === "/طلب_إستقدام" || url === "/إختيار_العمالة" ? (
            <div className="all-cv">
                ...
                return (
                  <div className="cv" key={worker._id}>
                    ...
                    <div className="cv-bottom">
                      ...
                      <button onClick={() => handleDownload(worker._id)}>
                        حجز السيرة الذاتية
                      </button>
                    </div>
                  </div>
              ...
              {getWorkers.length === 0 ? (
                <p className="not"> لا يوجد عمال بهذه البيانات </p>
              ) : (
                ""
              )}
            </div>
            ...
            <div className="card">
              <div className="header">
                <div>
                  <img src="/imgs/logo.png" alt="" className="avatar" />
              ...
              <div className="buttons">
                <button className="bookButton">احجز الآن</button>

                <button className="cvButton">تنزيل السيرة</button>
              </div>
            </div>
          ) : (
            ""
